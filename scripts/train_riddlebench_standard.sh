#!/bin/bash
# Ablation 1: Standard GRPO advisor for RiddleBench.
# No diagnosis format, single initial response, R_outcome only.
...
CKPT_PATH="${CKPT_PATH:-$HOME/ckpts/riddlebench_standard}"
RUN_NAME="${RUN_NAME:-riddlebench_standard}"
...
"$SKYRL_PYTHON" -m advisor_models.riddlebench.main_riddlebench \
  ...
  environment.env_class=riddlebench_standard \
