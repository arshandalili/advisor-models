...
class SkyRLGymGenerator(GeneratorInterface):
    ...
    async def agent_loop(
    ...
    ) -> Tuple[List[int], float, str, List[int], List[int], Optional[List[float]], Dict[str, Any]]:
        """
        ...
        response_outputs = []  # collect response text for logging
        ...
        while not done:
            ...
            response_outputs.append(output)  # collect for logging
        ...
        if self.use_conversation_multi_turn:
            prompt_text = "".join(m["content"] for m in chat_history)
            # Use collected response outputs instead of chat_history which may not be updated
            response_text = "".join(response_outputs)
            ground_truth = env_step_output["metadata"].get("ground_truth", "")
            final_response = env_step_output["metadata"].get("updated_response", "")
            initial_reward = env_step_output["metadata"].get("initial_reward", "")
            rollout_metrics["trajectories"] = [(prompt_text), (response_text), (final_response), ground_truth, reward, initial_reward]
            
            # (aczhu) additional logging for monte carlo reward estimation
            # individual_rewards = env_step_output["metadata"].get("individual_rewards", "")
            # rollout_metrics["trajectories"] = [(prompt_text), (response_text), (final_response), ground_truth, reward, initial_reward, individual_rewards]
        ...
        return response_ids, reward, stop_reason, loss_mask, prompt_ids, rollout_logprobs, rollout_metrics
    ...
    async def generate(self, input_batch: GeneratorInput) -> GeneratorOutput:
        ...
        individual_rollout_metrics = [output[6] for output in all_outputs]
