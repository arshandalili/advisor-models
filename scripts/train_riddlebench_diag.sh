#!/bin/bash
# Ablation 2: Diagnosis format + k=2 inputs for RiddleBench.
# Adds structured <diagnosis><advice> format and k=2 student responses.
# Still R_outcome only (no multi-component reward, no counterfactual).
...
CKPT_PATH="${CKPT_PATH:-$HOME/ckpts/riddlebench_diag}"
RUN_NAME="${RUN_NAME:-riddlebench_diag}"

echo "[train] Diag advisor — model=$MODEL, gpus=$NUM_GPUS"
...
"$SKYRL_PYTHON" -m advisor_models.riddlebench.main_riddlebench \
  ...
  environment.env_class=riddlebench_diag \
